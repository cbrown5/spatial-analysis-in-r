...
Leaflet makes use of a Javascript package for mapping (this is the language that dynamic web pages tend to use). It builds maps of your data ontop a range of freely available map layers.  Check out this guide to  [leaflet in R](https://rstudio.github.io/leaflet/) for more examples.  
...
We can do a bit more with leaflet maps than this. One option is to change the tiles. See a full list of options  [here](http://leaflet-extras.github.io/leaflet-providers/preview/index.html).  We can also colour the markers by the species richness.  
